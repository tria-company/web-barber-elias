/**
 * Tabela de Máquinas (Figma 23-5055).
 * Colunas: NOME, CÓDIGO, RECEBIMENTO (DÉBITO), RECEBIMENTO (CRÉDITO), STATUS (toggle), AÇÃO.
...
export interface MaquinaRow {
  ...
  nome: string;
  codigo: string;
  recebimentoDebito: number;
  recebimentoCredito: number;
  status: boolean;
}

export interface MaquinaTableProps {
  data: MaquinaRow[];
  onToggleStatus?: (id: string, value: boolean) => void;
  /** Quando informado, "Visualizar" abre o modal em vez de navegar. */
...
export function MaquinaTable({ data, onToggleStatus, onVisualizar }: MaquinaTableProps) {
  const [localStatus, setLocalStatus] = useState<Record<string, boolean>>(() =>
    data.reduce((acc, row) => ({ ...acc, [row.id]: row.status }), {})
  );

  const getStatus = (id: string) =>
    localStatus[id] ?? data.find((r) => r.id === id)?.status ?? false;
        ...
        <tbody className="divide-y divide-gray-200 bg-white">
          {data.map((row) => (
            <tr key={row.id} className="hover:bg-gray-50">
              ...
              <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">
                {row.recebimentoDebito}
              </td>
              <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">
                {row.recebimentoCredito}
              </td>
              <td className="whitespace-nowrap px-6 py-4">
                <Toggle
                  checked={getStatus(row.id)}
                  onChange={(v) => handleToggle(row.id, v)}
                  id={`status-${row.id}`}
                  ariaLabel={`Máquina ${row.nome} ativa`}
                />
              </td>
              <td className="whitespace-nowrap px-6 py-4">
                {onVisualizar ? (
                  ...
                  <Link
                    href={`/dashboard/cadastro/maquina/${row.id}`}
                    className="text-sm font-medium text-[var(--color-primary)] hover:underline"
                  >
                    Visualizar
                  </Link>
